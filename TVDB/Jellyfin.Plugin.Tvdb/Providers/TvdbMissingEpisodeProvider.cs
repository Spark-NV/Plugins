...
namespace Jellyfin.Plugin.Tvdb.Providers
{
    ...
    public class TvdbMissingEpisodeProvider : IHostedService
    {
        ...
        private void OnLibraryManagerItemRemoved(object? sender, ItemChangeEventArgs itemChangeEventArgs)
        {
            ...
            // Don't recreate episodes that had actual files (stub files) - these should be handled by the refresh logic
            if (itemChangeEventArgs.Item is Episode removedEpisode && !string.IsNullOrEmpty(removedEpisode.Path))
            {
                _logger.LogDebug("Skip: Removed episode had a file path (stub file), let refresh logic handle recreation");
                return;
            }
