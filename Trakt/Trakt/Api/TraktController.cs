...
public class TraktController : ControllerBase
{
    ...
    /// <summary>
    /// Get user's Trakt lists.
    /// </summary>
    /// <param name="userGuid">The user's GUID.</param>
    /// <response code="200">Lists returned.</response>
    /// <returns>A <see cref="List{TraktList}"/> with user's lists.</returns>
    [HttpGet("Users/{userGuid}/Lists")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<ActionResult<List<TraktList>>> GetUserLists([FromRoute] Guid userGuid)
    {
        ...
        var lists = await _traktApi.SendGetUserListsRequest(traktUser).ConfigureAwait(false);
        return lists ?? new List<TraktList>();
    }
    ...
    /// </summary>
    /// <param name="userGuid">The user's GUID.</param>
    /// <param name="listId">The list ID or slug.</param>
    /// <response code="200">List items returned.</response>
    /// <returns>A <see cref="List{TraktListItem}"/> with list items.</returns>
    [HttpGet("Users/{userGuid}/Lists/{listId}/Items")]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<ActionResult<List<TraktListItem>>> GetListItems([FromRoute] Guid userGuid, [FromRoute] string listId)
    {
        var traktUser = UserHelper.GetTraktUser(userGuid, true);
        if (traktUser == null)
        {
        ...
        var items = await _traktApi.SendGetListItemsRequest(traktUser, listId).ConfigureAwait(false);
        return items ?? new List<TraktListItem>();
    }

    /// <summary>
    /// Create stub files from a Trakt list.
    /// </summary>
    /// <param name="userGuid">The user's GUID.</param>
    /// <response code="200">Stub files created.</response>
    /// <returns>The number of stub files created.</returns>
    [HttpPost("Users/{userGuid}/CreateStubs")]
    [Authorize(Policy = Policies.RequiresElevation)]
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<ActionResult<object>> CreateStubFiles([FromRoute] Guid userGuid)
    {
        var traktUser = UserHelper.GetTraktUser(userGuid, true);
        if (traktUser == null)
        {
        ...
        {
            return BadRequest(new { error = "No list selected" });
        }

        if (string.IsNullOrWhiteSpace(traktUser.StubMovieLibraryPath) && string.IsNullOrWhiteSpace(traktUser.StubTvLibraryPath))
        {
            return BadRequest(new { error = "No library path specified for movies or TV shows" });
        }

        var listItems = await _traktApi.SendGetListItemsRequest(traktUser, traktUser.SelectedListId).ConfigureAwait(false);
        if (listItems == null || listItems.Count == 0)
        {
            return Ok(new { count = 0, message = "No items found in list" });
        }

        var loggerFactory = HttpContext.RequestServices.GetRequiredService<ILoggerFactory>();
        var stubHelper = new StubFileHelper(loggerFactory.CreateLogger<StubFileHelper>());
        var count = stubHelper.CreateStubFiles(listItems, traktUser.StubMovieLibraryPath, traktUser.StubTvLibraryPath);

        return Ok(new { count, message = $"Created {count} stub files" });
    }
}
